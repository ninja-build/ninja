...
- `-j1` (no parallelism) is not used on the command line.
  Note that `-j0` means "infinite" parallelism and does not disable
  client mode.
...
IMPORTANT: Ninja itself does not implement a jobserver pool.
However, the https://github.com/ninja-build/ninja/blob/master/misc/jobserver_pool.py[jobserver_pool.py]
Python3 script, available from its source repository, can be used to setup a
pool then start any multi-build command or script with it.
