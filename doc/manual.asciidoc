...
The `NINJA_SMART_TERMINAL` environment variable controls the terminal behaviour
when displaying progress. The default behaviour is `on`.
There are 3 options:

`off`:: Disable all smart terminal behaviour.
Each progress notification appears on a new line
`on`:: Enable smart terminal behaviour.
Progress notifications overwrite the same terminal line
`hide_newline`:: Smart terminal behaviour that ends each update 
with \n newline + move up 1 line
Useful if the ninja progress is piped to a line processing tool like grep or sed.
