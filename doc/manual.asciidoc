...
`description`:: a short description of the command, used to pretty-print
  ...
  line will always be printed before the command's output (except if `-F`
  is present).

`flush`:: if present, causes Ninja to flush the output of the command when it
  is not built in parallel. Useful for rule calling a long running command
  with a long output like documentation generation, tests execution, or
  configuration stage. The `-F` flag controls whether to flush all command
  outputs. When the command output is not buffered and if the command fails,
  the full command line will be printed after the command's output.
