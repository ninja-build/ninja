...
featureful than Ninja.  For some recommendations: the Ninja
...
of CMake support generating Ninja files on Windows and macOS too.
...
`-k N`::
  Keep going until _N_ jobs fail (0 means infinity; default is 1).

`-l N`::
  Do not start new jobs when the system load average is greater than _N_.
...
`$description`:: The current edge's `description` (or its command, when the
description is empty or `--verbose` is in effect). Unlike `NINJA_STATUS`,
which always appends the description after the formatted status, `--status`
only shows the description where `$description` appears in the format. Omit
it to drop the description entirely; place it wherever you want it shown.
...
a generated file, but do not have a proper dependency (possibly transitive)
...
Variables might be better called "bindings", in that a given variable
...
and save a compact form of the same information in a Ninja-internal
database.
...
`rspfile`, `rspfile_content`:: if present (both), Ninja will use a
  ...
  command.
+
This is particularly useful on Windows, where the maximum length of
...
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
Fundamentally, command lines behave differently on Unix and Windows.

On Unix systems, commands are arrays of arguments.  The Ninja `command`
