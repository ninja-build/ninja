...
+
The broken targets can be found assuming dynamic (depfile) dependency
...
generated file.
+
Additionally, look for files inside the build dir that do not have a generator
target listing them as outputs, and that are a deps log dependency of some
other target. These cases are also potentially problematic and they prevent
the main missing dependency check from working.
+
By default the tool assumes all files under the build dir are generated, so
in-source builds will have false positives for all static sources. To avoid
that, pass a -g DIR option to the tool to choose a directory relative to the
build dir that will be treated as the "generated files only" directory. An
empty string there disables the missing generator target check.
