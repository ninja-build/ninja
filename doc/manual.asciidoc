...
Heavy branch back-and-forth switching or rebasing often leads to a lot
of touched files that are actually not dirty and their dependent targets
are not outdated. Ninja supports these workflows with hash based input
checking. Enabling this feature for a rule will consider an input dirty
only if the content of an input file changed since the last successful build.
