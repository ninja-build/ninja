...
`--scheduler-trace FILE` records scheduler decisions using stable graph
identities. `--scheduler-replay FILE` validates such a trace against the
current manifest without executing build commands. Tracing is disabled by
default. Failure injection, causal reduction, format details, and the
compatibility policy are documented in
https://github.com/ninja-build/ninja/blob/master/doc/scheduler_trace.md[scheduler_trace.md].
...
`trace-reduce`:: derive a replayable causal slice for a failed stable edge
identity. Use +ninja -t trace-reduce _input_ _edge-id_ _output_+.
