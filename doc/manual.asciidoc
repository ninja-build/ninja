...
[[ref failed_cmd_script]]
The failed commands script
~~~~~~~~~~~~~~~~~~~~~~~~~~

_Available since Ninja 1.7._

For convenience, Ninja generates a shell script in the build directory
called `.ninja_failed_commands.sh` containing the command that failed
during the last build. You can edit and run this script to investigate
what part of a long command is wrong. In dry run mode the script is
left untouched.
