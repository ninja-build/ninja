...
working directory (determined as documented by the
<<ref_toplevel, top-level variable>> `ninja_workdir`).
...
`ninja_workdir`:: _(Available since Ninja 1.11.)_ If present, sets the
  ...
  See also the <<ref_headers, discussion of header dependencies>>.
