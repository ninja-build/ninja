...
Loading the same file additional times has no effect (similar to the C
`#pragma once` statement). However, one consistently use `include` or `subninja`
when loading the same file.

If there is a build statement for a loaded file, then `ninja` will update it
