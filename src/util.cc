...
int ReadFile(const std::string& path, std::string* contents, std::string* err) {
  ...
  if (f == INVALID_HANDLE_VALUE) {
    err->assign(GetLastErrorstd::string());
    return -ENOENT;
  }
...
void SetCloseOnExec(int fd) {
  ...
  if (! SetHandleInformation(hd, HANDLE_FLAG_INHERIT, 0)) {
    fprintf(stderr, "SetHandleInformation(): %s", GetLastErrorstd::string().c_str());
  }
#endif  // ! _WIN32
}
...
#ifdef _WIN32
std::string GetLastErrorstd::string() {
...
void Win32Fatal(const char* function) {
  Fatal("%s: %s", function, GetLastErrorstd::string().c_str());
}
#endif
