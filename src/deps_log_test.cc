...
TEST_F(DepsLogTest, WriteRead) {
  ...
  {
    vector<Node*> deps;
    deps.push_back(state1.FindOrCreateDepfileNode("foo.h", 0));
    deps.push_back(state1.FindOrCreateDepfileNode("bar.h", 0));
    log1.RecordDeps(state1.FindOrCreateDepfileNode("out.o", 0), 1, deps);

    deps.clear();
    deps.push_back(state1.FindOrCreateDepfileNode("foo.h", 0));
    deps.push_back(state1.FindOrCreateDepfileNode("bar2.h", 0));
    log1.RecordDeps(state1.FindOrCreateDepfileNode("out2.o", 0), 2, deps);

    DepsLog::Deps* log_deps = log1.GetDeps(state1.FindOrCreateDepfileNode("out.o", 0));
  ...
  DepsLog::Deps* log_deps = log2.GetDeps(state2.FindOrCreateDepfileNode("out2.o", 0));
  ASSERT_TRUE(log_deps);
...
TEST_F(DepsLogTest, LotsOfDeps) {
...

  {
    ...
    for (int i = 0; i < kNumDeps; ++i) {
      ...
      deps.push_back(state1.FindOrCreateDepfileNode(buf, 0));
    }
    log1.RecordDeps(state1.FindOrCreateDepfileNode("out.o", 0), 1, deps);
  ...
  DepsLog::Deps* log_deps = log2.GetDeps(state2.FindOrCreateDepfileNode("out.o", 0));
  ASSERT_EQ(kNumDeps, log_deps->node_count);
}
...
TEST_F(DepsLogTest, DoubleEntry) {
  ...
  int file_size;
  {
    ...
    vector<Node*> deps;
    deps.push_back(state.FindOrCreateDepfileNode("foo.h", 0));
    deps.push_back(state.FindOrCreateDepfileNode("bar.h", 0));
    log.RecordDeps(state.FindOrCreateDepfileNode("out.o", 0), 1, deps);
    log.Close();
...
"build other_out.o: cc\n";
  ...
  int file_size;
  {
    ...
    deps.clear();
    deps.push_back(state.FindOrCreateDepfileNode("foo.h", 0));
    deps.push_back(state.FindOrCreateDepfileNode("baz.h", 0));
    log.RecordDeps(state.FindOrCreateDepfileNode("other_out.o", 0), 1, deps);
  ...
  int file_size_3;
  {
    ...
    Node* out = state.FindOrCreateDepfileNode("out.o", 0);
    ...
    Node* other_out = state.FindOrCreateDepfileNode("other_out.o", 0);
...
TEST_F(DepsLogTest, Truncated) {
  // Create a file with some entries.
  {
    ...
    deps.clear();
    deps.push_back(state.FindOrCreateDepfileNode("foo.h", 0));
    deps.push_back(state.FindOrCreateDepfileNode("bar2.h", 0));
    log.RecordDeps(state.FindOrCreateDepfileNode("out2.o", 0), 2, deps);
...
TEST_F(DepsLogTest, TruncatedRecovery) {
  ...
  // Load the file again, add an entry.
  {
    ...
    EXPECT_EQ(NULL, log.GetDeps(state.FindOrCreateDepfileNode("out2.o", 0)));
    ...
    deps.push_back(state.FindOrCreateDepfileNode("foo.h", 0));
    deps.push_back(state.FindOrCreateDepfileNode("bar2.h", 0));
    log.RecordDeps(state.FindOrCreateDepfileNode("out2.o", 0), 3, deps);
  ...
  // entry doesn't break things.
  {
    ...
    DepsLog::Deps* deps = log.GetDeps(state.FindOrCreateDepfileNode("out2.o", 0));
    ASSERT_TRUE(deps);
  }
}
...
TEST_F(DepsLogTest, ReverseDepsNodes) {
  ...
  Node* rev_deps = log.GetFirstReverseDepsNode(state.FindOrCreateDepfileNode("foo.h", 0));
  EXPECT_TRUE(rev_deps == state.FindOrCreateDepfileNode("out.o", 0) ||
              rev_deps == state.FindOrCreateDepfileNode("out2.o", 0));

  rev_deps = log.GetFirstReverseDepsNode(state.FindOrCreateDepfileNode("bar.h", 0));
  EXPECT_TRUE(rev_deps == state.FindOrCreateDepfileNode("out.o", 0));
}
...
TEST_F(DepsLogTest, MalformedDepsLog) {
  std::string err;
  {
    ...
    deps.push_back(state.FindOrCreateDepfileNode("foo.hh", 0));
    deps.push_back(state.FindOrCreateDepfileNode("bar.hpp", 0));
    log.RecordDeps(state.FindOrCreateDepfileNode("out.o", 0), 1, deps);
    log.Close();
  }
