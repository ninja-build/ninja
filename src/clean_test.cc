...
"build out2: cat in | out1\n"
  ...
  // First use the manifest that describe how to build out1.
  DepsLog deps_log1;
  Cleaner cleaner1(&state, config_, &fs_, &deps_log1);
  EXPECT_EQ(0, cleaner1.CleanDead(log2.entries()));
  EXPECT_EQ(0, cleaner1.cleaned_files_count());
  EXPECT_EQ(0u, fs_.files_removed_.size());
  EXPECT_NE(0, fs_.Stat("in", &err));
  ...
  // Then use the manifest that does not build out1 anymore.
  DepsLog deps_log2;
  Cleaner cleaner2(&state_, config_, &fs_, &deps_log2);
  EXPECT_EQ(0, cleaner2.CleanDead(log2.entries()));
  EXPECT_EQ(0, cleaner2.cleaned_files_count());
  EXPECT_EQ(0u, fs_.files_removed_.size());
  EXPECT_NE(0, fs_.Stat("in", &err));
...
"build out: cp-plus-bis in\n"
  ...
  Cleaner cleaner(&state_, config_, &fs_, &deps_log);
  EXPECT_EQ(0, cleaner.CleanAll());
  EXPECT_EQ(3, cleaner.cleaned_files_count());
  EXPECT_EQ(3u, fs_.files_removed_.size());

  EXPECT_EQ(0, fs_.Stat("out", &err));
  EXPECT_EQ(0, fs_.Stat("out.bis", &err));
  EXPECT_EQ(0, fs_.Stat("out.dynout", &err));
...
TEST_F(CleanTest, CleanDynamicOutputForDirtyNode) {
  // Even if the node is dirty, the cleaner should use the dynamic outputs from
  // the deps log. It should not attempt to build the node to update the dynamic
  // output set.
  ASSERT_NO_FATAL_FAILURE(AssertParse(&state_,
"rule cp-plus-bis\n"
"  command = cp $in $out && cp $in $out.bis\n"
...
"build out: cp-plus-bis in\n"
  ...
  string err;
  Node* out = state_.LookupNode("out");
  ASSERT_TRUE(out->StatIfNecessary(&fs_, &err));
  ASSERT_EQ("", err);
  ...
  Node* out_bis = state_.GetNode("out.bis", 0);
  nodes.push_back(out_bis);
  deps_log.RecordDeps(out, out->mtime(), nodes, 1);

  fs_.Tick();
  fs_.Create("in", "");
  Node* in = state_.LookupNode("in");
  ASSERT_TRUE(in->StatIfNecessary(&fs_, &err));
  ASSERT_EQ("", err);
