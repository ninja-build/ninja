...
bool ImplicitDepLoader::LoadImplicitOutputs(Edge* edge, string* err) {
  ...
  if (!dynout.empty())
    return LoadDynOutFile(state_, disk_interface_, edge, dynout, err);
...
bool ImplicitDepLoader::LoadDynOutFile(State* state, DiskInterface* disk_interface, Edge* edge, const string& path,
  ...
  auto find = [](const string& s, size_t pos) -> size_t {
