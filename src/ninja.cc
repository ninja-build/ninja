...
struct NinjaMain : public BuildLogUser {
  ...
  /// spell correction.
  Node* CollectTarget(const char* cpath, string* err) const;
  ...
  graph::Options ToolGraphGroups(const Options* options, int argc,
                                            char* argv[],
                                            std::set<Node*>& topLevelNodes,
                                            int& err) const;
  /// Open the build log.
...
Node* NinjaMain::CollectTarget(const char* cpath, string* err) const{
  string path = cpath;
  if (path.empty()) {
    *err = "empty path";
    return NULL;
...
/// @err no error if set to zero
graph::Options NinjaMain::ToolGraphGroups(
    const Options* options, int argc, char* argv[],
    std::set<Node*>& topLevelNodes, int& err) const {
  const char* helptext =
      "Usage: ninja -t graph [options] [targets] [options] [targets] ...\n"
      "\n"
      "Generate a Graphviz DOT file representing the build graph for the given targets.\n"
      "The graph is constructed by iterating over each target and following its inputs\n"
      "You can filter or group targets to reduce output size and focus on specific parts.\n"
      "Grouped Options apply independently to each group of targets.\n"
      "\n"
      "Examples:\n"
      "  ninja -t graph mytarget | dot -Tsvg -ograph.svg\n"
      "  ninja -t graph -x '.*substring.*' mytarget\n"
      "  ninja -t graph -r myTarget1 myTarget2 myTarget3\n"
      "  ninja -t graph -r myTarget1 myTarget2 -d 0 -o myTarget3 @default\n"
      "\n"
      "Global Options:\n"
      "  -h, --help              Show this help message.\n"
      "  -E, --elide=LEN         Limit label length to LEN characters (default: unlimited).\n"
      "  -p, --disable-dyndep    Do not load dyndep files.\n"
      "  -P, --enable-dep        Force loading of dep files.\n"
      "  -S, --enable-scan       Load deps and dyndeps using the standard ninja algorithm.\n"
      "  -T, --statistic         Print statistics about the exported graph.\n"
      "\n"
      "Grouped Options (apply per target group):\n"
      "  -d, --depth=DEPTH       Limit depth of dependency sub-tree.\n"
      "  -o, --no-order-only     Exclude order-only dependencies.\n"
      "  -i, --no-implicit       Exclude implicit dependencies.\n"
      "  -e, --no-explicit       Exclude explicit dependencies.\n"
      "  -D, --no-gen-depload    Exclude nodes generated by deps-loader.\n"
      "                          (effective only if deps/dyndeps are loaded;\n"
      "                           recommended when deps are loaded)\n"
      "  -x, --exclude-target=RE Exclude targets matching regex RE.\n"
      "  -s, --no-siblings       Exclude sibling targets.\n"
      "\n"
      "Traverse Modes (choose maximum one per group):\n"
      "  -R, --reverse           Traverse graph in reverse order.\n"
      "  -r, --relations         Show all relations between targets.\n"
      "                          (ignores -s and -d)\n"
      "  -Q, --query             Behave similar to 'ninja -t query'.\n"
      "\n"
      "Special Targets:\n"
      "  @root                   All targets with no outputs.\n"
      "  @default                Default targets.\n"
      "                          (equivalent to 'ninja -t graph' without args)\n";
  err = 0;  // no error
  ...
  const option kLongOptions[] = { { "help", no_argument, NULL, 'h' },
                                  ...
                                  { "exclude-target", required_argument, NULL, 'x' },
                                  { "no-order-only-deps", no_argument, NULL, 'o' },
                                  { "no-implicit-deps", no_argument, NULL, 'i' },
                                  { "no-explicit-deps", no_argument, NULL, 'e' },
                                  { "no-gen-depload", no_argument, NULL, 'D' },
                                  { "no-siblings", no_argument, NULL, 's' },
                                  { "elide", required_argument, NULL, 'E'},
                                  { "relation", no_argument, NULL, 'r' },
                                  { "disable-dyndep", no_argument, NULL, 'p' },
                                  { "enable-dep", no_argument, NULL, 'P' },
                                  { "enable-scan", no_argument, NULL, 'S' },
                                  { "statistic", no_argument, NULL, 'T' },
                                  { "query", no_argument, NULL, 'Q' },
                                  { NULL, 0, NULL, 0 } };


  graph::Options groups;
  bool myQuery = false;

  // defined as a lambda, to inhibit duplicating code twice
  auto closeGroup = [&argc = C17::as_const(argc), &argv = C17::as_const(argv),
                     &groups, &topLevelNodes, this](
                        graph::Group& myGroup, bool& query, bool& groupClosed) {
    int idx = optind;
    std::string err;
    while (idx < argc && argv[idx][0] != '-') {
      const bool defaultT(strcmp(argv[idx], "@default") == 0);
      const bool rootT(strcmp(argv[idx], "@root") == 0);
      if (defaultT || rootT) {
        std::vector<Node*> nodes;
        nodes = defaultT ? state_.DefaultNodes(&err) : state_.RootNodes(&err);
        if (!err.empty()) {
          Error("%s", err.c_str());
          return 1;
        }

        // add group with default targets
        for (const auto node : nodes) {
          myGroup.targets_.insert(node);
          topLevelNodes.insert(node);
        }
      } else {
        Node* myTarget = CollectTarget(argv[idx], &err);
        if (myTarget == nullptr) {
          Error("%s", err.c_str());
          return 1;
        }
        myGroup.targets_.insert(myTarget);
        topLevelNodes.insert(myTarget);
      }
      idx++;
    }
    if (!myGroup.targets_.empty()) {
      // group is closed
      groups.pushGroup(myGroup, query);
      myGroup = graph::Group();
      query = false;
      groupClosed = true;
    } else {
  ...
  bool isgroupClosed = false;
  {
    //  targets at the beginning without options
    //  "ninja -t graph target1 ..."
    graph::Group myGroup;
    bool query = false;
    auto ret = closeGroup(myGroup, query, isgroupClosed);
    if (ret != 0) {
      err = ret;
      return {};
    }
  ...
  // 2. New group with defaults
  graph::Group myGroup;
  myQuery = false;
  opterr = 0;  // suppress getopt_long error messages

  while ((opt = getopt_long(argc, argv, "+d:x:+E:sSDoTpPQrRieh0", kLongOptions,
                            NULL)) != -1) {
    switch (opt) {
    case 's':
      myGroup.options_.input_siblings_ = false;
      break;
    case 'x':
      myGroup.options_.regexExclude_ = optarg;
      break;
    case 'e':
      myGroup.options_.exportExplicitLinks_ = false;
      break;
    case 'E':
      groups.middleElide_ = atoi(optarg);
      if (groups.middleElide_ > 0)
        groups.middleElide_ += 3;
      break;
    case 'i':
      myGroup.options_.exportImplicitLinks_ = false;
      break;
    case 'D':
      myGroup.options_.exportGenDepLoader_ = false;
      break;
    case 'o':
      myGroup.options_.exportOrderOnlyLinks_ = false;
      break;
    case 'r':
      myGroup.options_.relations_ = true;
      break;
    case 'R':
      myGroup.options_.reverse_ = true;
      break;
    case 'd':
      myGroup.options_.depth = atoi(optarg);
      break;
    case 'p':
      groups.loadDyndep_ = false;
      break;
    case 'P':
      groups.loadDep_ = true;
      break;
    case 'S':
      groups.scan_ = true;
      break;
    case 'T':
      groups.statistic_ = true;
      break;
    case 'Q':
      myQuery = true;
      break;
    ...
    // Collect all following targets for current group
    auto ret = closeGroup(myGroup, myQuery, isgroupClosed);
    if (ret != 0) {
      err = ret;
      return {};
    }
    ...
    if (!myGroup.options_.checkRegex()) {
      Error("Invalid regex expression for option -x: \"%s\".",
            myGroup.options_.regexExclude_.c_str());
      err = 1;
      return {};
    }
  }
  ...
  if (!isgroupClosed) {
    ...
    groups.pushGroup(myGroup, myQuery);
  }
...
int NinjaMain::ToolGraph(const Options* options, int argc, char* argv[]) {
  // analyze cli and return the groups
  int err = 0;
  std::set<Node*> topLevelNodes; // for propagate dyndep
  graph::Options groups =
      ToolGraphGroups(options, argc, argv, topLevelNodes, err);
  if (err != 0)
    return err;

  // loading dyndeps and deps
  if (groups.loadDyndep_ || groups.loadDep_)
    ActivateDyndep(&state_, &disk_interface_, &deps_log_, topLevelNodes,
                   groups.loadDyndep_, groups.loadDep_);

  if (groups.scan_) {
    DependencyScan scan(&state_, &build_log_, &deps_log_, &disk_interface_,
                        nullptr, nullptr);
    std::vector<Node*> validation_nodes_dummy;

    std::string error;
    for (auto target : topLevelNodes)
      if (!scan.RecomputeDirty(target, &validation_nodes_dummy, &error)) {
        Error(error.c_str());
        return 1;
      }
  }
  ...
  // export the graph
  if (groups.statistic_)
    GraphViz::printStatistics(groups);
  else
    GraphViz::printDot(groups);
...
const Tool* ChooseTool(const string& tool_name) {
  static const Tool kTools[] = {
    ...
    { "graph", "output graphviz dot file for targets",
      Tool::RUN_AFTER_LOGS, &NinjaMain::ToolGraph },
