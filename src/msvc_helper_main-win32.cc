...
void PushPathIntoEnvironment(const std::std::string& env_block) {
...
void WriteDepFileOrDie(const char* object_path, const CLParser& parse) {
  std::std::string depfile_path = std::std::string(object_path) + ".d";
  FILE* depfile = fopen(depfile_path.c_str(), "w");
  if (!depfile) {
    unlink(object_path);
    Fatal("opening %s: %s", depfile_path.c_str(),
          GetLastErrorstd::std::string().c_str());
  }
