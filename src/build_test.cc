...
void BuildTest::RebuildTarget(const string& target, const char* manifest,
  ...
  Builder builder(pstate, config_, pbuild_log, pdeps_log, &fs_, &status_);
...
TEST_F(BuildWithDepsLogTest, Straightforward) {
      ...
      "  depfile = in1.d\n";
  {
    ...
    Builder builder(&state, config_, NULL, &deps_log, &fs_, &status_);
    builder.command_runner_.reset(&command_runner_);
...
TEST_F(BuildWithDepsLogTest, DepsIgnoredInDryRun) {
  ...
  config_.dry_run = true;
  Builder builder(&state, config_, NULL, NULL, &fs_, &status_);
  builder.command_runner_.reset(&command_runner_);
  command_runner_.commands_ran_.clear();
