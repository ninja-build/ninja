...
bool Lexer::ReadEvalString(EvalString* eval, bool path, string* err) {
  ...
  for (;;) {
    ...
    /*!re2c
    [^$ :\r\n|\000]+ {
      eval->AddText(StringPiece(start, p - start).Persist(arena_));
      continue;
    }
    "\r\n" {
      if (path)
    ...
    [ :|\n] {
      if (path) {
        ...
        if (*start == '\n')
          break;
        eval->AddText(StringPiece(start, 1).Persist(arena_));
        continue;
      }
    }
    "$$" {
    ...
    "${"varname"}" {
      eval->AddSpecial(StringPiece(start + 2, p - start - 3).Persist(arena_));
      continue;
    }
    "$"simple_varname {
      eval->AddSpecial(StringPiece(start + 1, p - start - 1).Persist(arena_));
      continue;
    }
    "$:" {
      eval->AddText(StringPiece(":", 1));
