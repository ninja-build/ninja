...
class DyndepFileSorted {
 public:
  ...
  const std::vector<const Node*>* findOut(const Edge* edge) const;
  /// Returns the edge that produces the given node via dyndep.
  const Edge* findOut(const Node* node) const;

  const std::vector<const Node*>* findIn(const Edge* edge) const;

 private:
  // Each node maps to the edge that produces it.
  const std::unordered_map<const Node*, const Edge*> nodeSortedOut_;

  // Each edge maps to its output nodes.
  const std::unordered_map<const Edge*, std::vector<const Node*>>
      edgeSortedOut_;

  // Each edge maps to its input nodes.
  const std::unordered_map<const Edge*, std::vector<const Node*>> edgeSortedIn_;
...
/// Loads all dyndep files referenced in the graph and emits warnings for any
/// dyndeps that are reachable from the given set of nodes.
///
...
/// Get all warnings from reachable loadings, unit test only
std::vector<std::string> GetWarnings(State* state,
